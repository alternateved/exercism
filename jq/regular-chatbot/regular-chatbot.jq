...
#
def is_valid_command: test("^chatbot\\W"; "i") ;
...
def check_phone_number:
  . as $phone | test("[(+]\\d{2}[)] \\d{3}-\\d{3}-\\d{3}") |
    if . then
      "Thanks! Your phone number is OK."
    else
      "Oops, it seems like I can't reach out to \($phone)."
    end
;
...
def get_domains: [ scan("\\b[a-z]+\\.[a-z]+\\b") ];
...
def nice_to_meet_you: capture("name is \\b(?<name>[a-zA-Z-]+)\\b") | "Nice to meet you, \(.name)";
