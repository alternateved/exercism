...
| (.actual_minutes_in_oven // 0) as $in_oven
| ($expected_minutes_in_oven - $in_oven) as $remaining_minutes_in_oven
| (.number_of_layers // 1) as $layers
| ($layers * 2) as $preparation_time
| ($in_oven + $preparation_time) as $total_time
| { 
